...
export default function PortalSidebar({ certificates, letters }: PortalSidebarProps) {
  const t = useTranslations("Dashboard.portal.sections");
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-xl font-black font-outfit text-slate-900">{t("certificates.title")}</h3>
          <Link href="/portal/certificates" className="text-xs font-bold text-slate-400 hover:text-primary transition-colors">{tc("viewAll")}</Link>
        </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-xl font-black font-outfit text-slate-900">{t("letters.title")}</h3>
          <Link href="/portal/letters" className="text-xs font-bold text-slate-400 hover:text-primary transition-colors">{tc("viewAll")}</Link>
        </div>
      ...
      <div className="p-8 rounded-[24px] bg-slate-900 text-white space-y-4 relative overflow-hidden group shadow-xl">
        <div className="relative z-10">
          <h4 className="text-xl font-bold font-outfit">{t("help.title")}</h4>
          <p className="text-slate-400 text-sm font-medium leading-relaxed">{t("help.desc")}</p>
          <button className="mt-4 px-6 py-3 bg-white/10 hover:bg-white/20 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">{t("help.button")}</button>
        </div>
