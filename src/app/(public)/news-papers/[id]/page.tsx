...
export default async function NewsPaperViewPage({ params }: { params: { id: string } }) {
  const paper = await getNewsPaperById(params.id);
  ...
  if (!pdfUrl) {
    return (
        <div className="min-h-screen pt-32 pb-20 flex flex-col items-center justify-center space-y-4">
            <h1 className="text-3xl font-black font-outfit text-primary">PDF Not Available</h1>
            <p className="text-foreground/60">The document for this issue is currently unavailable.</p>
            <Link href="/news" className="text-primary hover:underline font-bold mt-4">
                &larr; Back to News
            </Link>
        </div>
    );
           ...
           <div className="flex items-center gap-4 mt-4 text-sm font-semibold text-foreground/60 uppercase tracking-widest">
              ...
              {paper.issue_number && (
                 <>
                   ...
                   <span className="text-primary bg-primary/10 px-3 py-1 rounded-full">Issue {paper.issue_number}</span>
                 </>
              )}
           </div>
        </div>
        ...
        <div className="bg-white border border-border/50 rounded-[32px] p-2 md:p-6 shadow-2xl shadow-primary/5 h-[80vh] min-h-[600px] flex overflow-hidden">
             ...
             >
                <div className="flex flex-col items-center justify-center h-full space-y-4">
                   <p className="text-foreground/60 font-medium">Your browser does not support inline PDFs.</p>
                   <a href={pdfUrl} className="text-primary font-bold hover:underline py-2 px-4 rounded-xl bg-primary/5">
                      Download the PDF
                   </a>
                </div>
             </object>
