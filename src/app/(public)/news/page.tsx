...
// Helper function to extract first image URL from markdown
...
async function getVideos() {
    try {
        ...
        return data.filter((item: any) => item.is_published);
    } catch (err) {
            ...
            <section className="relative min-h-[60vh] flex items-center justify-center overflow-hidden py-32 px-6 bg-slate-950">
                ...
                <div className="max-w-4xl mx-auto relative z-20 text-center space-y-8">
                    <div className="p-12 lg:p-16 rounded-[60px] bg-white/5 border border-white/10 backdrop-blur-2xl space-y-6 shadow-2xl">
                        ...
                        <h1 className="text-5xl lg:text-7xl font-black font-outfit text-white tracking-tighter leading-none">News & <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-300 to-amber-500 italic">Press</span></h1>
                            ...
                            return (
                                <div key={i} className="bg-white border border-border rounded-[32px] overflow-hidden hover:shadow-2xl hover:shadow-primary/5 transition-all group flex flex-col">
                                    ...
                                    <div className="p-6 space-y-3 flex-1 flex flex-col">
                                        <h3 className="text-xl font-bold font-outfit text-primary line-clamp-2">{video.title}</h3>
                                        {video.description && (
                                            <p className="text-sm text-foreground/60 line-clamp-2">{video.description}</p>
                                        )}
                                        <div className="pt-4 mt-auto border-t border-border/50">
                                            <p className="text-xs font-bold text-foreground/30 uppercase tracking-widest">{new Date(video.created_at).toLocaleDateString()}</p>
                                        </div>
                                    </div>
                                </div>
                            );
